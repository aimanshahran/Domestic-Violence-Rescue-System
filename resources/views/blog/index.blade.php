        ...
        <div class="card card-2">
            <div class="container pt-5">
                ...
                <table class="table table-bordered table-striped table-hover" style="width:100%;border: none;">
                    ...
                    <tr>
                        <th scope="col" style="width:10%;text-align: center;vertical-align: middle;">{!! __('blog.idB') !!}</th>
                        <th scope="col" style="width:40%;text-align: center;vertical-align: middle;">{!! __('blog.nameB') !!}</th>
                        <th scope="col" style="width:20%;text-align: center;vertical-align: middle;">{!! __('blog.writerB') !!}</th>
                        <th scope="col" style="width:10%;text-align: center;vertical-align: middle;">{!! __('blog.last_updatedB') !!}</th>
                        <th scope="col" style="width:10%;text-align: center;vertical-align: middle;">{!! __('blog.editB') !!}</th>
                        <th scope="col" style="width:10%;text-align: center;vertical-align: middle;">{!! __('blog.delB') !!}</th>
                    </tr>
                    </thead>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered modal-s" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h3>{!! __('blog.warning') !!}</h3>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <div class="modal-body text-center">
                                                <p>{!! __('blog.delWarn') !!}{{ $post->id }}?</p>
                                                <div class="mt-4"><button type="submit" class="btn btn-success btn-lg">{!! __('blog.y') !!}</button>&nbsp<button type="button" class="btn btn-danger btn-lg" data-dismiss="modal" aria-label="Close">{!! __('blog.n') !!}</button></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @empty
                        <td colspan="6" style="text-align: center">{!! __('blog.noB') !!}.{{ date_format(now(), "d-m-Y H:i:s") }}</td>
                    @endforelse
                </table>
                @else
                    <h2>Blog</h2>
                    @forelse($posts as $post)
                        <div class="row align-items-center">
                            ...
                            <div class="col-offset-* my-auto">
                                <p>{!! __('blog.posted_at') !!}{{date_format($post->created_at, "d/m/Y")}}</p>
                            </div>
                        </div>
                        <div class="row align-items-center">
                        ...
                        <div class="row align-items-center">
                            <div class="col" style="padding-bottom: 30px">
                                <form action="{{route('blog.show', $post->id)}}" method="get">
                                    @csrf
                                    <button type="submit" class="btn btn-primary" style="background-color: #6F42C1; border: transparent;">{!! __('blog.readMore') !!}</button>
                                </form>
                            </div>
                        </div>
                    @empty
                        <p class="text-warning">{!! __('blog.noBavailable') !!}</p>
                    @endforelse
                    {!! $posts->links() !!}
            </div>
            @endadmin_writer
